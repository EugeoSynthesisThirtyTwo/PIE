...
void Grid::serialize(BetterVector<char>& buffer) const
{
	...
	s::serialize<uint32_t>(buffer, width);
	s::serialize<uint32_t>(buffer, height);
...
Grid::Grid(const char* buffer, int& cursor)
{
	...
	width = s::unserialize<uint32_t>(buffer, cursor);
	height = s::unserialize<uint32_t>(buffer, cursor);
